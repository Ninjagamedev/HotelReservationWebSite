...
if (isset($_POST['submit'])) {
  ...
  echo $filename;
    ...
    $stmt3 = $conn->prepare("UPDATE atividade SET Thumbnail=? Where ID = ?");
...
$sql = "UPDATE atividade SET Nome=?, Categoria=?, Cidade=?,Pais=?, Descricao=?, Morada=?, Classificacao=?,Preco=?,Horario=? WHERE ID=?";
$stmt=$conn->prepare($sql);
$stmt->bind_param('ssssssiisi',$NomeAlojamento,$Categoria,$Cidade,$Pais,$Descricao,$Morada,$Classificacao,$Preco,$Horario,$last_id);
